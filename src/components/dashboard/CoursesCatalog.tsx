...
import { activeTracks, upcomingTracks, moduleStats, moduleLessons, Track } from "@/data/curriculum";
...
const TrackSection: React.FC<{
  track: Track;
  onSelectModule: (moduleId: string) => void;
}> = ({ track, onSelectModule }) => {
  const { moduleProgress, isCompleted } = useProgress();

  const trackPercent = useMemo(() => {
    const allLessons = track.modules.flatMap((m) => moduleLessons(m));
    if (allLessons.length === 0) return 0;
    const done = allLessons.filter((l) => isCompleted(l.lesson.id)).length;
    return Math.round((done / allLessons.length) * 100);
  }, [track.modules, isCompleted]);
  ...
  const themeColor = track.colorTheme || "#26B54F";

  return (
    <section className="mb-6 last:mb-4">
      ...
      <div className="relative bg-[#F8F9FA] dark:bg-[#1F1F1F] rounded-[15px] border border-gray-200/80 dark:border-zinc-800/80 p-4 sm:p-5 shadow-2xs">
        ...
        >
          {track.modules.map((mod, idx) => {
            ...
            const accentColor = mod.accent || themeColor;
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {mp.isFinished ? (
                      <IconCircleCheckFilled size={18} style={{ color: accentColor }} />
                    ) : mp.isStarted ? (
...
export const CoursesCatalog: React.FC<CoursesCatalogProps> = ({
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 font-sans">
      ...
      {/* ── ACTIVE TRACKS (with modules & isSoon = false) ── */}
      ...
      {/* ── UPCOMING TRACKS (isSoon = true or no modules) ── */}
